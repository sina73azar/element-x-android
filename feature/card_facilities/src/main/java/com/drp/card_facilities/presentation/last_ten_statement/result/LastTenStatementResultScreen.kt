...
import com.drp.utils.organizeDate
...
fun StatementItemWidget(modifier: Modifier = Modifier, bankStatement: BankStatement) {
    ElevatedCard(
        ...
        Row(
            modifier = Modifier.padding(vertical = dimensionResource(id = UiRes.dimen.medium_padding)),
            verticalAlignment = Alignment.CenterVertically
        ) {
            IconButton(
                modifier = Modifier
                    .padding(
                        vertical = dimensionResource(id = UiRes.dimen.small_padding)
                    ),
                onClick = {}) {
//                Icon(
...
//                )
            ...
            Column(
                modifier = Modifier
                    .weight(1f)
                    .padding(
                        end = dimensionResource(id = UiRes.dimen.medium_padding),
                        start = 4.dp
                    ),
            ) {
                Row(
                    ...
                    Text(
                        modifier = Modifier
                            .wrapContentWidth()
                            .padding(start = dimensionResource(id = UiRes.dimen.medium_padding)),
                        text = currencyFormatter(
                            bankStatement.amount.toLong()
                        ) + " " + stringResource(
                            id = UiRes.string.irr_currency
                        ),
                        maxLines = 1,
                        overflow = TextOverflow.Ellipsis,
                        style = MaterialTheme.typography.bodyMedium.copy(
