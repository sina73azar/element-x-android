package com.drp.card_facilities.presentation.card_to_card.compose.inquiry
...
import com.drp.card_facilities.presentation.card_to_card.cardToCardReceipt
...
import com.drp.card_facilities.presentation.card_to_card.compose.transfer.CardToCardTransferBottomSheet
...
import com.drp.shared_ui.widget.CardNumberScanner
...
fun CardToCardInquiryScreen(
    ...
    Scaffold(snackbarHost = {
        ...
        CustomTopAppBar(
            headerTxt = stringResource(id = R.string.card_to_card_st),
            onBackClick = {
            ...
            ElevatedCard(
                ...
                CardNumberScanner(
                    modifier = Modifier.padding(
                        top = dimensionResource(id = com.drp.shared_ui.R.dimen.large_padding)
                    )
                ) { exposedCardNumber ->
                    viewModel?.setDestinationCard(
                        exposedCardNumber
                    )
                }
                PanEditText(
                    ...
                    value = uiState.destinationCard,
                    onValueChange = {
                        viewModel?.setDestinationCard(it)
                    },
                    contactSheetList = uiState.destinationCardContactSheetList,
                    spinnerListContact = uiState.destinationCardContactSpinner,
                    onDismissSpinner = {
                        viewModel?.dismissDestinationCardContactSpinner()
                    },
                    onSpinnerDropDownClick = {
                        viewModel?.setDestinationCard(it)
                    },
                    keyboardType = KeyboardType.Text,
                    errorMessage = uiState.destinationCardValidationMessage.asString(),
                    dumpErrorMessage = { viewModel?.dumpDestinationCardValidationMessage() }
                )
                ...
                CardFacilityAmountEditText(
                    ...
                    dumpErrorMessage = {
                        viewModel?.dumpAmountValidationMessage()
                    })
                ...
                SearchSheet(
                    sheetVisible = monthSelectionSheet,
                ...
                SearchSheet(
                    sheetVisible = yearSelectionSheet,
                ...
                /** card to card inquiry bottom sheet */
                CardToCardTransferBottomSheet(
                    visibility = uiState.inquiryBottomSheetVisibility,
                    viewModel = viewModel,
                    receiptItems = cardToCardReceipt(
                        sourceCardNumber = uiState.sourceCardUiState.selectedCard?.pan ?: "",
                        destinationCardNumber = uiState.destinationCard.filter { it.isDigit() },
...
@Preview
@Composable
private fun CardToCardInquiryScreenPreview() {
    ApplicationTheme {
