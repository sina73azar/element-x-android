package com.drp.card_facilities.presentation.wallet_minus.wirhdraw
...
import com.drp.card_facilities.presentation.wallet_minus.WalletMinusScreenEvents
import com.drp.card_facilities.presentation.wallet_minus.WalletMinusScreenState
...
@Composable
fun WithdrawFromWalletBottomSheet(
    ...
    DynamicSheet(
        ...
        title = stringResource(
            id = R.string.decrease_balance_st
        ),
        ...
        LazyColumn {
            ...
            item {
                OTPPasswordEditTextCompose(
                    ...
                    otpSmsRequest = {
                        viewModel?.sendEvent(
                            WalletMinusScreenEvents.SendOtp
                        )
                    },
                    smsState = uiState.walletUiState.walletSmsState,
                    textValue = uiState.walletUiState.walletOtpCode,
                    onValueChange = { otpCode ->
                        viewModel?.sendWalletEvent(WalletEvents.SetOtpCode(otpCode = otpCode))
                    },
                    maxLength = 12,
                    errorMessage = uiState.walletUiState.walletOtpCodeValidationMessage.asString(),
                    dumpErrorMessage = { viewModel?.sendWalletEvent(WalletEvents.DumpOtpCodeValidationMessage) },
                    showError = {
                        viewModel?.sendEvent(
                            WalletMinusScreenEvents.ShowError(
                                it
                            )
                        )
                    }
            ...
            item {
                SubmitCancelRowCompose(
                    ...
                    onSubmitClick = {
                        viewModel?.sendEvent(
                            WalletMinusScreenEvents.Transfer
                        )
...
@Composable
private fun WithdrawFromWalletBottomSheetPreview() {
    ApplicationTheme {
