...
@HiltViewModel
class MotorViolationViewModel @Inject constructor(
    ...
    private val _uiState = MutableStateFlow(MotorViolationScreenState())
    val uiState: StateFlow<MotorViolationScreenState> = _uiState
    ...
    private fun validateFields(): Boolean {
        if (uiState.value.leftNumber.length != 3) {
            _uiState.value =
                _uiState.value.copy(leftNumberError = true)
            return false
        }
        if (uiState.value.rightNumber.length != 5) {
            _uiState.value =
    ...
    fun inquiry() {
        ...
        viewModelScope.launch(dispatcher) {
            ...
            with(uiState.value) {
                cardFacilitiesRepository.motorViolationInquiry(
                    ...
                    it.toRequestState().let { response ->
                        if (response.isError()) {
                            showError(UiText.StringResource(R.string.motor_violation_inquiry_error_st))
                        }
